...
;; selected? is Boolean value describes whether or not the doodad is selected
...
(define SELECTED-STAR-DOODAD1-PLACE-DOODAD
    (make-doodad "gold" (radial-star 8 10 50 "solid" "gold")
               (make-location 125 120) (make-velocity 10 12)
               (make-mouse-position 125 120) 0 false))
...
(define WORLD-PLACE-SELECTED2-DOODAD (make-world
          (list SELECTED-STAR-DOODAD-PLACE-DOODAD)
          '()4 0 #false))

(define IMAGE-OF-SELECTED2-DOODAD
...
;; EXAMPLES:
;; (world-after-key-event WORLD-AT-INITIAL-POSITIONS "t") = WORLD-AFTER-T-KEY
;; a star doodad is added to the world
;; (world-after-key-event WORLD-AT-INITIAL-POSITIONS "q") = WORLD-AFTER-Q-KEY
;; a square doodad is added to the initial world
;; (world-after-key-event WORLD-BEFORE-DELETE-OLDEST-STAR ".") =
;;                                           WORLD-AFTER-DELETE-OLDEST-STAR
;; the world returned will be the world with the oldest star(s) and oldest
;; square(s) deleted.
;; (world-after-key-event WORLD-STAR-SELECTED "c") =
;;                                           WORLD-STAR-COLOR-CHANGE-SELECTED
;; the world is returned with the colours of the selected doodads changed.
;; (world-after-key-event WORLD-AT-INITIAL-POSITIONS " ") = WORLD-ON-PAUSE
;; the world returned is paused.
...
(define (world-after-key-dot w)
   (make-world
    (doodads-after-key-dot (world-doodads-star w))
    (doodads-after-key-dot (world-doodads-square w))
...
(define (doodads-after-key-dot lst)
  (filter
   ;; Doodad -> Boolean
   ;; returns true if it is not the oldest doodad else returns false if the
   ;; doodad is the oldest doodad.
   (lambda (doodad) (not(= (doodad-age doodad) (find-oldest-age lst)))) lst))
...
;; RETURNS: an Integer, the oldest age of the doodads in the given list
;; EXAMPLES:
;; (find-oldest-age (list DOODAD4 DOODAD3 DOODAD2 DOODAD1 DOODAD-OLDEST-AGE)= 10
;; STRATEGY: Use template for LOD-square or LOD-star on lst

(define (find-oldest-age lst)
  (doodad-age (first(reverse lst))))
...
;; RETURNS: a LOD,with each selected doodad change in color
;; EXAMPLES:
;; (doodads-after-key-c (list DOODAD4 DOODAD3 DOODAD2 SELECTED-STAR-DOODAD))
;;   = (list DOODAD4 DOODAD3 DOODAD2 SELECTED-STAR-DOODAD-CHANGE-COLOR)
...
(define (doodads-after-key-c lst)
  (map
   ;; Doodad -> Doodad
   ;; returns the doodad after changing the color of the doodad
   ;; if the doodad is selected else the same doodad is returned
   ;; without any change in color
   (lambda (doodad)
          (doodad-after-c doodad)) lst))
...
;; EXAMPLES:
;; (doodad-after-c SELECTED-STAR-DOODAD) = SELECTED-STAR-DOODAD-CHANGE-COLOR
;; (doodad-after-c DOODAD3) = DOODAD3
...
(define (doodad-after-c doodad)
  ...
  (make-doodad (change-color (doodad-color doodad))
               ...
               (doodad-age doodad)(doodad-selected? doodad)) doodad))
...
(begin-for-test
  ...
  (check-equal? (world-to-scene WORLD-PLACE-SELECTED2-DOODAD)
                IMAGE-OF-SELECTED2-DOODAD
                "The world hasn't been created properly"))
...
;; RETURNS: a Scene,  that portrays the given world.
;; EXAMPLE:
;; (place-doodads (list INITIAL-STAR-DOODAD)) = IMAGE-OF-INITIAL-WORLD
;; the initial star doodad is placed on the empty canvas
;; STRATEGY: Use HOF foldr on lst

(define (place-doodads lst scene)  
  (foldr
   ;; Doodad Scene -> Scene
   ;; Doodad is placed on the Scene
   (lambda (doodad scene) (place-doodad doodad scene)) scene lst))

;; place-doodad: Doodad Scene -> Scene
;; GIVEN: a Doodad,either a star doodad or square doodad.
;; RETURNS: a Scene,  that portrays the given world.
;; EXAMPLE:
;; (place-doodad INITIAL-STAR-DOODAD) = IMAGE-OF-INITIAL-WORLD
...
;; EXAMPLE:
;; (world-after-tick WORLD-ON-PAUSE) = WORLD-AFTER-TICK-ON-PAUSE
;; STRATEGY:Use template for World on w
...
;; EXAMPLE:
;; (increase-age WORLD-ON-PAUSE) = WORLD-AFTER-TICK-ON-PAUSE
;; the age of the doodads in the world is increased by 1 for each tick.
...
;;          function and the age of each doodad is incremented by one.
;; EXAMPLES:
;; (doodads-increase-age (list INITIAL-STAR-DOODAD)) =
;;     = (list DOODAD-STAR-ON-PAUSE-AFTER-TICK) 
;; STRATEGY: Use HOF map on lst
...
(define (doodads-increase-age lst)
  (map
   ;; Doodad -> Doodad
   ;; returns a Doodad after increasing its age by 1
   (lambda (doodad) (doodad-increase-age doodad)) lst))

;; doodad-increase-age: Doodad -> Doodad
;; GIVEN: a Doodad, either a star-like doodad or square-like doodad
;; RETURNS: a Doodad, the age of the doodad is incremented by one and returned.
;; EXAMPLE:
;; (doodad-increase-age INITIAL-STAR-DOODAD) = DOODAD-STAR-ON-PAUSE-AFTER-TICK
...
;;          returned.
;; EXAMPLE:
;; (doodads-after-tick (list XY-LESS-THAN-ZERO-BEFORE)) =
;;                 (list XY-LESS-THAN-ZERO-AFTER)
;; STRATEGY: Use HOF map on lst
...
(define (doodads-after-tick lst)
  (map
   ;; Doodad -> Doodad
   ;; returns a Doodad after a tick
   (lambda (doodad) (doodad-after-tick doodad)) lst))
...
(define (doodad-after-tick doodad)
  (if (doodad-selected? doodad)
      ...
      (cond
    [(and(and (>= (tentative-x doodad) 0)(< (tentative-x doodad) CANVAS-WIDTH))
        (and (>= (tentative-y doodad) 0)(< (tentative-y doodad) CANVAS-HEIGHT)))
    ...
    [(>= (tentative-x doodad) CANVAS-WIDTH)
          (doodad-at-x-boundary doodad)]
    [(>= (tentative-y doodad) CANVAS-HEIGHT)
...
;; RETURNS: the LOD after following the given mouse event
;; EXAMPLE:
;; (doodads-after-mouse-event (list INITIAL-STAR-DOODAD) 125 120 "button-down")=
;;            (list INITIAL-STAR-DOODAD-BUTTON-DOWN)
;; STRATEGY: Use HOF map on lst
...
(define (doodads-after-mouse-event lst mx my mev)
  (map
   ;; Doodad -> Doodad
   ;; returns a Doodad after the mouse event
   (lambda (doodad) (doodad-after-mouse-event doodad mx my mev)) lst))
...
;; EXAMPLE:
;; (doodad-after-mouse-event INITIAL-STAR-DOODAD 125 120 "button-down") =
;;                      INITIAL-STAR-DOODAD-BUTTON-DOWN
...
;; EXAMPLE:
;; (doodad-after-button-down INITIAL-STAR-DOODAD 125 120) =
;;                   INITIAL-STAR-DOODAD-BUTTON-DOWN
;; STRATEGY: Use template of Doodad on doodad
...
;; RETURNS: a Doodad, which is either a square or star
;; EXAMPLE:
;; (doodad-after-drag (list INITIAL-SQUARE-DOODAD) 125 120) =
;;  INITIAL-SQUARE-DOODAD-BUTTON-DRAG
;; STRATEGY: Use template of Doodad on doodad
...
;; RETURNS: a Doodad, which is either a square or star
;; EXAMPLE:
;; (doodad-after-button-up INITIAL-STAR-DOODAD-BUTTON-DRAG 125 120) =
;;    INITIAL-STAR-DOODAD-BUTTON-UP
;; STRATEGY: Use template of Doodad on doodad
...
;; (in-doodad? DOODAD-SQUARE-FOR-IN-DOODAD-BEFORE 384 184) =
;;    DOODAD-SQUARE-FOR-IN-DOODAD-AFTER
