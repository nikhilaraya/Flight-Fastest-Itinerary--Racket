...
;; A List of List of Flapjacks (LOLOF) is either
;; -- empty
;; (cons LOF LOLOF)
...
;; overlapping-flapjacks : ListOfFlapjack(LOF) -> ListOfListOfFlapjack(LOLOF)
;; GIVEN: a list of flapjacks
;; RETURNS: a list of list of flapjacks. Each flapjack checks with the other
;;          flapjacks in the list if it is overlapping with the other flapjacks.
;;          If the flapjack overlaps with the other flapjacks then a list of all
...
(define (overlapping-flapjacks lst)
  (map
   ;; Flapjack -> ListOfFlapjacks
   ;; returns a list of flapjacks which overlap with the given flapjack
   (lambda (flapjack) (check-flapjack-with-flapjacks flapjack lst)) lst))
...
;; checking-overlapping-flapjacks : LOF LOF -> ListOFListOfFlapjack(LOLOF)
...
(define (check-flapjack-with-flapjacks flapjack lst)
  (filter
   ;; Flapjack -> Boolean
   ;; returns true if the two flapjacks are overlapping and returns false if
   ;; the two flapjacks are not overlapping
   (lambda (first-flapjack)
            (overlapping-condition-flapjack? flapjack first-flapjack)) lst))
...
(define (non-overlapping-flapjacks lst)
  (filter
   ;; Flapjack -> Boolean
   ;; returns true if the length of the list of overlapping flapjacks for a
   ;; particular flapjack is 1 else returns false
   (lambda (flapjack)
     (= (length (check-flapjack-with-flapjacks flapjack lst)) 1)) lst)) 
...
(define (flapjacks-in-skillet lst skillet)
  (filter
   ;; Flapjack -> Boolean
   ;; returns true if the flapjack fits into the skillet completely else
   ;; returns false
   (lambda (first-flapjack)
            (check-for-flapjack-in-skillet? first-flapjack skillet)) lst))
